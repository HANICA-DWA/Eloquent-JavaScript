...
* A guided tour through some aspects of the reduxyfied RrHN-client
* Discuss, and explain to students, how to program with immutability in JS.
* Let students finish the PreferencesDialog in the 'reduxyfied' RrHN-client

The reduxyfied RrHN-client is a version with the AJAX and React-router stuff removed. It does have the PreferencesDialog, and it keeps track of read/seen/new-statuses of items. But the HN-items and the initial item-statuses are read from the same static files students were using earlier.

There are two versions:

1. A basic version, that does not implement the Redux-code for the PreferencesDialog yet.  
   This version will be made available to the students. This version can also be shown to the students during the guided tour. It uses some of my own helper-functions for creating updated versions of immutable datastructures.
2. A finished version of the basic one. This one is for the teacher, but you can supply it to students before they have to implement AJAX.


## Part 1: The guided tour

👉 Start a the dev-server for the client, and show the app working, but that the PreferencesDialog is dysfunctional (can be shown, can't be dismissed). Mention that:

*  the PreferencesDialog will be hooked up by the students themselves.
*  Note that AJAX is subject for the next session.

#### Start by looking at  `index.jsx`:
...
*  The large commented-out block attaches the Redux DevTools, *and* installs a logging middleware to make sure that Actions are also displayed in the normal console (because Redux Devtools does not show error messages).
   *  ​👉 Simply uncomment, en remove the simple `createStore` line. 
...
*  Pay attention to the `<Provider>` in the JSX. Redux wants its Provider to be the top-level component. When a `<Router>` must be wrapped around the app, the `<Router>` will go *inside* the `<Provider>`. 
   The Provider is just a trick to get React to make the store available to all lower-level components. 
*  Mention that Redux *can* be used with other libraries (e.g. Angular, or on the server). Almost everyone uses it with React, though. But this independence is the reason why Redux needs a helper library like `ReactRedux` to integrate with React.

​👉 Switch to the file `reducers.js`
...
*  Do a quick glance at the `hnItemsReducer`  function itself. Don't explain the code yet. Just point to the fact that all branches of the switch return the state (usually updated, but the default branch is important).
*  Point to the last 4 lines, where the reducers get combined.
   *  Emphasise that each reducer handles its own part of the app-state, and has no access to the state as a whole. That is why the `hnItemsReducer` accesses `state.selectedItem` instead of `state.hnItems.selectedItems` (compare with `App.js`, line 63). 
...
*  Emphasize how much more compact this code has become. It still has some logic (choosing bewteeen iframe and prefs-dialog). One main reason is that `App` no longer passes a lot of props to `ItemList` and `PreferencesDialog`. :question:Why? Answer: Those components hook themselves up to the Redux store, and therefore don't need to get the data and functions from the parent.
...
*  Feel free to admit that we don't know why the `ReactRedux.connect()()`-call has this weird curried structure. I've tried to find out — did not succeed.
...
#### Switch to the file `ItemList.jsx`
...
*  Note the use of a function called `mapDispatchToProps()` to provide the eventhandlers.
   *  This version is different from the one in the video's: there, a helper-function called ``bindActionCreators()` is used. My code basically shows what `bindActionCreators()`: it wraps all Action creators inside a call to `dispatch()` to **turn action creators into event handler that change the Redux store**. `bindActionCreators()` is not needed in Redux code: just use the pattern shown here.
...
#### reducer-code: working with immutability
...
This is the moment to switch discussion to the scond part of the workshop:

## Part 2: Programming with immutability in JS

Here's how I would do an explanation of immutability in JS:

*  Numbers and booleans are immutable: you can only replace a number with a new one.

*  Immutability in JS is about _changing parts of a datastructure_, not about replacing one structure with a completely different one.

*  Strings are funny: They have parts (booleans don't), but you can't change the parts: string in JS (and Java) are immutable. All methods in JS that change strings return a new string. The old one is still the same.

*  The only mutable things in JS are arrays and objects.

*  Redux wants immutability to enable the time-traveling debugger, and to prevent some types of programming error.

*  Reducers **_may not_** change an existing array or object in the state.

*  Use memory-models to explain what a _shallow_ copy is.

*  Give an example of how you would deal with updating a _nested_ datastructure (changing a field in an object in an array in an object). E.g.   
...
*  setting the grade of SWD to '9.5' would require code like this:  
...
👉Show the students the "preferencesReducer". 
...
*  Explain that even for controlled inputs, the state must be kept in the Redux store. Otherwise you wouldn't be able to use the time-traveling debugger for problems with user-behaviour while editing values in the dialog.
